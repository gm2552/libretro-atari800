...
int Retro_PollEvent()
{
   ...
   int i,j;
   static int vbt[4][16]={
		{0x40,0x0,0x0,0x0,0x01,0x02,0x04,0x08,0x80,0x40,0x0,0x0,0x0,0x0,0x0,0x0},
		{0x40,0x0,0x0,0x0,0x01,0x02,0x04,0x08,0x80,0x40,0x0,0x0,0x0,0x0,0x0,0x0},
		{0x40,0x0,0x0,0x0,0x01,0x02,0x04,0x08,0x80,0x40,0x0,0x0,0x0,0x0,0x0,0x0},
		{0x40,0x0,0x0,0x0,0x01,0x02,0x04,0x08,0x80,0x40,0x0,0x0,0x0,0x0,0x0,0x0},
	};
   ...
   for(j=0;j<4;j++){
      ...
      if( input_state_cb(j, RETRO_DEVICE_JOYPAD, 0, 0)) {
            MXjoy[j] |= vbt[j][0]; // Joy press
	    fprintf(stderr, "[Libretro-atari800]: MXjoy press controller %u slot %u.\n", j, 0);
      }
      else if( MXjoy[j]&vbt[j][0])
      {
            MXjoy[j] &= ~vbt[j][0]; // Joy press
      }
  ...
  if(a5200_joyhack) //hack for robotron right analog act as Joy1
  {
   ...
   //mbt[RETRO_DEVICE_ID_JOYPAD_START]=1;
